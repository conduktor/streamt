...
| Kafka Streams fixed Docker runner | Closed typed projection/filter SQL subset | Exact owned container, image, topic identity, status and progress checks; exact-operation diagnosis | Create managed jobs; no-op repeat apply; sole reviewed predicate update and explicit same-operation resume | Explicit `executor: kafka_streams`, immutable locally built image, local Unix-socket Docker daemon and bridge network only. One raw-JSON input/output; no joins, aggregation, windows or arbitrary JAR/image scheduling. Format-6 online full-project plan required for replacement. Projection/schema/identity/image/stateful changes, deletion and generic runner-failure recovery remain blocked. No implicit image pull, offset reset or external resource adoption. |
...
remains separate from the maintained runner. The public predicate-update and
same-operation-resume workflow has its own real Kafka/Docker acceptance, separate
from standalone Java clean-restart tests. Broader lifecycle transitions remain
blocked until their own acceptance gates pass.
