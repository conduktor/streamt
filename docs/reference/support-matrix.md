...
| PostgreSQL deployment state | No | Bounded status, confirmed initialization, and non-reserving lock diagnostics | No | Requires the optional `postgres` package extra. Status verifies the exact version-1 catalog; init atomically creates or registers an empty address. `lock-status` reports instantaneous `available`, `busy`, or `unregistered` from a primary and releases any transaction-scoped probe lock before success. PostgreSQL 14 and 18 run real-server and process-concurrency gates. Ordinary state authority, operation locking, ownership mutation, migration, and recovery remain disabled. |
