...
| PostgreSQL deployment state | No | Bounded status plus confirmed store/address initialization | No | Requires the optional `postgres` package extra. Status verifies the exact version-1 catalog in one read-only snapshot. Init atomically creates or registers an empty address, enforces the owner/ACL contract, and verifies it on a fresh connection. PostgreSQL 14 and 18 run real-server and process-concurrency gates. Lock probing, ordinary state authority, ownership mutation, migration, and recovery remain disabled. |
